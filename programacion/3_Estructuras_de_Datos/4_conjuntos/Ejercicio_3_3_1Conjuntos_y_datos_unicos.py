...
def obtener_domicilios_facturacion(compras):
    # Usamos set() porque no admite elementos repetidos
    domicilios_unicos = set()
    
    for compra in compras:
        # El domicilio está en la posición 3 de la tupla (cliente, día, monto, domicilio)
        domicilio = compra[3]
        domicilios_unicos.add(domicilio)
    
    return domicilios_unicos
...
print(f"Domicilios a los que enviar factura: {obtener_domicilios_facturacion(ventas_mes)}")